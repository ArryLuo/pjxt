  ...
  <div class="nav_banner_box"> 
    ...
    <!-- 登陆前显示-->
      ...
      <div class="inform">
        <h2>公告</h2>
        <ul>
           [@cms_content_list channelId='255' count='4' orderBy='updatetime desc'] 
			[#list tag_list as a]
...
<div class="w_1002">
  ...
  <div class="arrange">
    ...
    <div class="arrange_list">
      <ul>
        [@cms_course page=0 rows=10 orderStatus='2' courseType='foshanybgs100401']
	    [#if course_tag.rows??]
      ...
      <ul>
        [@cms_course page=0 rows=10 orderStatus='2' courseType='foshanybgs100402']
	    [#if course_tag.rows??]
			[#list course_tag.rows as a]
				<li>[#if a_index==0]<i class="orange">[#elseif a_index>0&&a_index<3]<i class="yellow">[#elseif a_index>2]<i>[/#if]
					${a_index+1}</i><a href="/course/courseDetails/${a.id}" title="${a.cname}">[@text_cut s=a.cname len=14 append='...'/]</a>
        ...
        [@cms_course_comment page=0 rows=10 dateFormat='yyyy-MM-dd']
			...
			<li>
			      [#if a.uimage ??]
                              ...
                              [#else]
                              <img src="/r/cms/gdybgs/default/images/grey_30x30/icon_tx.png" width="43" height="43" /><em></em>
                              [/#if]
